  ...
  , H.article_ [ P.className "s12 fill" ]
    ...
    , H.article_ [ P.className "grid" ]
      ( [ H.div_ [ P.className "s2" ] [ H.p_ [] [ H.b_ [] [ text "Attribute" ] ] ]
        ...
        <> (concatMap (mkAttributeRow x) allStats) 
        <> totalRow x
      )
...
mkAttributeRow x s =
    ...
    , H.div_ [ P.className "s2" ] [ H.p_ [ P.className "center-align" ] [ text (msshow cost) ] ]
    ]
  where
    total = (x ^. sl) + (x ^. rbl)
    mod = modifier total
...
totalRow x =
  [ H.div_ [ P.className "s2" ] [ H.button_ [ E.onClick ResetCustomValues ] [ text "Reset" ] ]
